...
describe file("#{user_dir}/.zshrc") do
  ...
  its('content') { should match %r{brew} }
  its('content') { should match %r{# BEGIN ZGENOM BLOCK}}
  its('content') { should match %r{if ! zgen saved} }
  its('content') { should match %r{zgen save$}}
  its('content') { should match %r{# END ZGENOM BLOCK}}
end
